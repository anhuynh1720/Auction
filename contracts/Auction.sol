...
contract Auction {
    ...
    // Register new Bidder
    function register(address _account, uint8 _token) valid(State.CREATED, true) public { 
    ...
    // Start the session.
    function startSession() valid(State.CREATED, true) public {
        state = State.STARTED;
    }
    

    
    function bid(uint8 _price) valid(State.STARTED, false) public {
    ...
    function announce() valid(State.STARTED, true) public {
    ...
    function getDeposit() valid(State.CLOSING, false) public {
    ...
    //modifiers
    modifier valid(State reqState, bool auctioneerRequire)
    { 
